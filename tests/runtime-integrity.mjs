...
assert.match(html, /css\/style\.css\?v=/, 'the runtime must keep one public stylesheet entry point');
...
const generator = readFileSync('js/modules/packlist-generator.js', 'utf8');
const pwa = readFileSync('js/modules/pwa.js', 'utf8');
const sw = readFileSync('sw.js', 'utf8');
const css = readFileSync('css/style.css', 'utf8');
const cssTokens = readFileSync('css/modules/tokens.css', 'utf8');
const cssBase = readFileSync('css/modules/base.css', 'utf8');
...
assert.match(app, /import\('\.\/modules\/admin\.js'\)/, 'the admin module must load only when the admin control is used');
...
assert.match(generator, /isWeatherCompatible\(item, config\.weather\)/, 'generated items must be filtered by per-item weather compatibility');
...
assert.match(generator, /isTransportCompatible\(item, config\.transports\)/, 'generated items must be filtered by per-item transport compatibility');
assert.match(generator, /Math\.ceil\(coveredDays/, 'quantity frequency must round up');
...
assert.match(generator, /isDepartureWornDailyItem/, 'daily base clothing must account for the item worn at departure');
...
assert.match(html, new RegExp(`\\.\\/vendor\\/pdf\\/packlist-pdf-adapter\\.js\\?v=${version}`), 'PDF export must load the local Packlist PDF adapter');
assert.match(html, new RegExp(`\\.\\/vendor\\/pdf\\/packlist-autotable-adapter\\.js\\?v=${version}`), 'PDF export must load the local Packlist AutoTable adapter');
...
assert.match(sw, new RegExp(`\\/vendor\\/pdf\\/packlist-pdf-adapter\\.js\\?v=${version}`), 'service worker must precache the local Packlist PDF adapter');
assert.match(sw, new RegExp(`\\/vendor\\/pdf\\/packlist-autotable-adapter\\.js\\?v=${version}`), 'service worker must precache the local Packlist AutoTable adapter');
